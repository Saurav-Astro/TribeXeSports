...
import { initiateGoogleSignIn, signInWithEmail } from "@/firebase/non-blocking-login";
...
const CardContentArea = ({ accentColor }: { accentColor: string }) => {
  ...
  return (
    <>
      <form className="grid gap-6" onSubmit={handleEmailPasswordSignIn}>
        <div className="grid gap-2">
          <Label htmlFor="email">Email Address</Label>
          <Input id="email" type="email" placeholder="your@email.com" required className="bg-zinc-900/60 border-zinc-800 h-12 text-base" value={email} onChange={(e) => setEmail(e.target.value)} />
        </div>
         <div className="grid gap-2">
          <Label htmlFor="password">Password</Label>
          <Input id="password" type="password" required className="bg-zinc-900/60 border-zinc-800 h-12 text-base" value={password} onChange={(e) => setPassword(e.target.value)} />
        </div>
        <Button type="submit" className="w-full h-12 text-lg font-bold transition-all duration-300" style={{backgroundColor: `hsl(${accentColor})`, textShadow: `0 0 15px hsl(${accentColor} / 0.8)`}}>
          Login
        </Button>
      </form>
      ...
      <div className="mt-6 text-center text-sm text-zinc-400">
        ...
        <Link href="/signup" className="underline font-semibold" style={{color: `hsl(${accentColor})`}}>
          Sign up
        </Link>
      </div>
    </>
        ...
        <div className="w-full flex-1 flex items-center justify-center bg-transparent">
             <Card className="w-full max-w-md glassmorphism">
                <CardHeader className="text-center">
                    <CardTitle className="text-4xl font-headline">Welcome Back</CardTitle>
                ...
                <CardContent>
                    <div className="animate-pulse space-y-6">
                        ...
                        <div className="h-12 w-full bg-zinc-700 rounded"></div>
                    </div>
                </CardContent>
             </Card>
        </div>
